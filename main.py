...
class MySignFrame(wx.Frame):
    ...
    def init_data(self):
        ...
        self.sql_name = os.path.join(BASE_DIR, 'punchcard.db')  # 数据库路径
    ...
    def initMenu(self):
        ...
        # 使用wx.Image()添加背景图片
        self.imageBackgroud = wx.Image(os.path.join(ASSETS_DIR, 'background.jpg'), wx.BITMAP_TYPE_ANY).Scale(500, 400)
        ...
        # 使用wx.Image()添加背景图片
        self.imageBackgroud1 = wx.Image(os.path.join(ASSETS_DIR, 'background.jpg'), wx.BITMAP_TYPE_ANY).Scale(500, 400)
	...
	*********************数据库操作end******************************
    ...
    def access_tokens(self):
        # 从环境变量获取百度AI API密钥，需在系统中设置 BAIDU_FACE_AK 和 BAIDU_FACE_SK
        ak = os.environ.get('BAIDU_FACE_AK', '')
        sk = os.environ.get('BAIDU_FACE_SK', '')
        if not ak or not sk:
            raise ValueError("请设置环境变量 BAIDU_FACE_AK 和 BAIDU_FACE_SK")
    ...
    def face_tracking(self):
        ...
        cap.set(4, 450)
        # 创建人脸检测器（快速Harr）
        face_cascade = cv2.CascadeClassifier(os.path.join(ASSETS_DIR, "haarcascade_frontalface_alt2.xml"))
            ...
            if (len(faces) > 0) & (time_interval >= 3):
                for (x, y, w, h) in faces:
                    if self.recognize == True:
                        ...
                        self._temp_face_path = os.path.join(BASE_DIR, '_temp_face.jpg')
                        cv2.imwrite(self._temp_face_path, roiImg)
                ...
                # 判断是否需要人脸质量检测
                if self.recognize == True:
                    self.picture_quality(self._temp_face_path)
                else:
                    if self.command == 'search':
                        if self.search == True:
                            self.face_search(self._temp_face_path)
                    elif self.command == 'registration':
                        if self.registration == False:
                            self.face_registration(self._temp_face_path, user)
